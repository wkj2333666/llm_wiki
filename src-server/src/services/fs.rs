...
///   3. OS dynamic loader search path (last resort)
...
fn extract_docx_with_library(path: &str) -> Result<String, String> {
    ...
    if result.trim().is_empty() {
        // Fallback: docx-rs returned nothing, try raw XML parsing.
        // Reuse the bytes already in memory instead of re-opening the file.
...
fn extract_docx_markdown<R: std::io::Read + std::io::Seek>(
    archive: &mut zip::ZipArchive<R>,
...
fn extract_pptx_markdown<R: std::io::Read + std::io::Seek>(
    archive: &mut zip::ZipArchive<R>,
) -> Result<String, String> {
...
fn extract_odf_text<R: std::io::Read + std::io::Seek>(
    archive: &mut zip::ZipArchive<R>,
) -> Result<String, String> {
...
fn build_tree(dir: &Path, depth: usize, max_depth: usize) -> Result<Vec<FileNode>, String> {
    ...
    for (entry, is_dir) in entries {
        ...
        // and compose paths consistently across Windows and Unix.
